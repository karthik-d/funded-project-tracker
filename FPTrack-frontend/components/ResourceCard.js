...
export default function ResourseCard(props) {
    ...
    const setCount_modified=(counter)=>{ if(counter<0 || counter>props.avl_qty)return; setCount(counter);}
    const handlesubmit = (e)=>{e.preventDefault(); if(count>0)alert(count);
    };
    return (
